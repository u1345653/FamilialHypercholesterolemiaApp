...
#Create a pinput table with balnk values
pinput = pd.DataFrame(columns=["age", "ldlc", "totalc", "gender", "clinicalCAD", "onsetAge", "tx", "dna_dx"],
                      data=[[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0],
                            [0,0,0,"f",0,0,0,0],[0,0,0,"f",0,0,0,0]])

# Creating all the calculations
...
def multiple():
    ...
    for i, j in pinput.iterrows():
        ...
        # print("ldlcnotfh called with " + str(ldlci) + " " + str(agei) + " " + str(totalci))
...
def user_input(input_value):
    ...
    pinput.loc[input_value] = [age, ldlc, totalc, gender, clinicalCAD, onsetAge, tx, dna_dx]
...
def stringlikestuff():
    ...
    # print(cadarray)
    # Calculating the String Probabilites
    #Creating the table
    probs = pd.DataFrame([FPP1(), FPP2(), FPP3(), TP4(), TP5(), TP6(), TP7(), TP8(), TP9(), LDLC1(), LDLC2(), LDLC3(), LDLC4(), LDLC5(), LDLC6(), LDLC7(),
                          LDLC8(), LDLC9(), TX1(), TX2(), TX3(), TX4(), TX5(), TX6(), TX7(), TX8(), TX9(), CAD1(), CAD2(), CAD3(), CAD4(), CAD5(), CAD6(),
                          CAD7(), CAD8(), CAD9()]).transpose()
    #Creating 4 sections
    section1 = pd.DataFrame([FPP1(), FPP2(), FPP3(), TP4(), TP5(), TP6(), TP7(), TP8(), TP9()]).transpose()
    section2 = pd.DataFrame([LDLC1(), LDLC2(), LDLC3(), LDLC4(), LDLC5(), LDLC6(), LDLC7(), LDLC8(), LDLC9()]).transpose()
    section3 = pd.DataFrame([TX1(), TX2(), TX3(), TX4(), TX5(), TX6(), TX7(), TX8(), TX9()]).transpose()
    section4 = pd.DataFrame([CAD1(), CAD2(), CAD3(), CAD4(), CAD5(), CAD6(), CAD7(), CAD8(), CAD9()]).transpose()
    # section1 = section1a.iloc[1:,:]
    # section2 = section2a.iloc[1:,:]
    # section3 = section3a.iloc[1:,:]
    # section4 = section4a.iloc[1:,:]
...
#Creating a menu for option selection
patient_family = { "0": "Add Grandmother on Parent #1 side", #Column 1
                   "1": "Add Grandfather on Parent #1 side", #Column 2
                   "3": "Add sibling of Parent #1", #Column 4
                   "4": "Add second sibling of Parent #1", #Column 5
                   "5": "Add Parent #1", #Column 6
                   "2": "Add Parent #2", #Column 3
                   "6": "Add Child of Parents 1 and 2", #Column 7
                   "7": "Add second Child of Parents 1 and 2", #Column 8
                   "8": "Add third Child of Parents 1 and 2", #Column 9
                   "9": "Compute the scores for FH and exit program"
                   }
...
while loop == True:
    print(" Options List ")
    for option in patient_family.items():
        print(option)

    patient_selection = input("Select an option: ")
    try:
        patient_int = int(patient_selection)
    except:
        patient_int = -1
        loop = False
